...
class ParamsBuilder(object):
    def __init__(
        ...
        self.need_set_epoch_hook = need_set_epoch_hook
    ...
    def get_train_data_params(self):
        ...
        train_data_params.update({
            ...
            'data_loader_kwargs': {
                'drop_last': True,
                'persistent_workers': PERSISTENT_WORKERS and self.persistent_workers,
                },
            })
        train_data_params['data_loader_kwargs'].update(
                self.add_train_loader_kwargs)
        self.params['train_data_params'] = train_data_params
        if (not self.shuffle) or (self.need_set_epoch_hook is not None):
